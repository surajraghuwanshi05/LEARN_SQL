...
-- 5.3 Calculate the number of days between the order_date and the delivery date for all orders.


SELECT order_id, DATEDIFF(delivery_date, order_date) AS days_to_deliver
FROM Orders;
-- 5.4 Retrieve the year, month, and day from the order_date column in the Orders table.


SELECT YEAR(order_date) AS year, MONTH(order_date) AS month, DAY(order_date) AS day
FROM Orders;

-- 5.5 Select all orders placed in the last 15 days using a date function.


SELECT *
FROM Orders
WHERE order_date >= CURDATE() - INTERVAL 15 DAY;
-- 5.6 Retrieve all products added to the Products table exactly one month ago from today.
...
-- 5.7 Add 10 days to the order_date for all orders and display the new date.

SELECT order_id, DATE_ADD(order_date, INTERVAL 10 DAY) AS new_order_date
FROM Orders;

-- 5.8 Subtract 2 months from the order_date column and display the result.


SELECT order_id, DATE_SUB(order_date, INTERVAL 2 MONTH) AS updated_order_date
FROM Orders;

-- 5.9 Select all orders where the difference between order_date and delivery_date is greater than 5 days.
